...
public class GatewayTraceFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        log.debug("[Trace] [Gateway] [{}] Start processing request", traceId);
        ...
        return chain.filter(mutatedExchange)
                .doOnSuccess(aVoid -> {
                    log.trace("[Trace] [Gateway] [{}] Request completed successfully", traceId);
                })
                .doOnError(throwable -> {
                    log.error("[Trace] [Gateway] [{}] Error occurred: {}", traceId, throwable.getMessage(), throwable);
                });
    }
    ...
    private String getOrGenerateTraceId(ServerWebExchange exchange) {
        ...
        if (traceId != null && !traceId.isEmpty()) {
            log.debug("[Trace] [Gateway] [{}] Extracted from request header", traceId);
            return traceId;
        }
        
        // 生成新的 Trace ID（16 位 UUID）
        traceId = TraceContext.generateTraceId();
        log.debug("[Trace] [Gateway] [{}] Generated new trace ID", traceId);
        return traceId;
    }
